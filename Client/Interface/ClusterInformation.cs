...
namespace Client.Interface
{
    ...
    public class ClusterInformation
    {
        ...
        /// <summary>
        /// Build from a list of <see cref="ServerDescriptionResponse"/> which may contain servers that did not respond to connection request
        /// </summary>
        ...
        public ClusterInformation(ServerDescriptionResponse[] serverDescriptions)
        {
            ...
            bool allGood = serverDescriptions.All(r => !r.ConnectionError);
        ...
        private void CheckClusterSanity(ServerDescriptionResponse[] responses)
        {
            ...
            if (ServersStatus.Length > 0)
            {
                ...
                for (int i = 1; i < ServersStatus.Length; i++)
                {
                ...
                if (Status == ClusterStatus.Ok)
                {
                    for (int i = 1; i < ServersStatus.Length; i++)
                    {
                        if (ServersStatus[i].ClusterName != clusterName)
                        {
                            ...
                            StatusReason = "The cluster name is not the same for all nodes. Probably configuration issue";
                            break;
                        }
                        ...
                        {
                            Status = ClusterStatus.Warning;
                            StatusReason = "The memory limit is not the same for all nodes. The cluster will work but the memory use will not be optimal";
                            break;
                        }
                    }
                }
