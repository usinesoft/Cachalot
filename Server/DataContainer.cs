...
public class DataContainer
{
    ...
    private Dictionary<string, int> _lastIdByGeneratorName = new();
    ...
    /// </summary>
    private SafeDictionary<string, DataStore> DataStores { get; } = new(null);
    ...
    private void ProcessLockRequest(LockRequest lockRequest, IClient client)
    {
        ...
        try
        {
            ...
            {
                var lockAcquired = lockManager.TryAcquireReadLock(lockRequest.SessionId,
                    Constants.DelayForLockInMilliseconds,
    ...
    private void ProcessDataRequest(DataRequest dataRequest, IClient client)
    {
        ...
        else
        {
            ...
            if (dataRequest.CollectionName.ToLower() == "@activity") dataStore = _serviceContainer.Log.ActivityTable;
        }


        if (dataStore == null) throw new NotSupportedException("Unknown collection : " + dataRequest.CollectionName);
    ...
    {
        if (DataStores.Count == 0) throw new CacheException("Can not backup an empty database");

        var lockManager = _serviceContainer.LockManager;

        // acquire a read lock on all the data stores
        lockManager.DoWithReadLock(() => { InternalDump(request, client); }, DataStores.Keys.ToArray());
    }
    ...
    private void InternalDump(DumpRequest request, IClient client)
    {
        try
        {
            ...
            var directory =
                $"{date.ToString("yyyy-MM-dd")}_{date.ToString("HH")}h{date.ToString("mm")}_{ShardCount:D2}nodes_{cluster}";
    ...
    private void GetKnownTypes(IClient client)
    {
        try
        {
            ...
            response.ServerProcessInfo = new()
            {
    ...
    public void LoadSchema(string path)
    {
        ...
        if (schema != null)
        {
            foreach (var description in schema.CollectionsDescriptions)
            {
                ...
                RegisterType(new(description.Value, schema.ShardIndex, schema.ShardCount), null);
            }
    ...
    {
        _lastIdByGeneratorName =
            _serviceContainer.SequencePersistence.LoadValues(path) ?? new Dictionary<string, int>();
    }
}