...
public class ClientConfig
{
    ...
    /// <summary>
    ///     Read from a connection string in the form host1:port1+host2:port2;max_connections_in_pool,
    ///     preloaded_connections_in_pool
    /// </summary>
    ...
    public ClientConfig(string connectionString)
    {
        ...
        if (connectionString.Contains(';'))
        {
            ...
            if (parts.Length > 0)
            {
                ...
                if (pool.Length > 1) PreloadedConnections = int.Parse(pool[1]);
            }
        }
        else
        {
        ...
        {
            if (!server.Contains(":")) throw new FormatException("A server should be specified as hostname:port");
            ...
            Servers.Add(new() { Host = host, Port = port });
        }
        ...
        if (Servers.Count > 1)
        {
            ...
            for (var i = 0; i < orderedServers.Count - 1; i++)
                if (orderedServers[i].Host == orderedServers[i + 1].Host &&
                    orderedServers[i].Port == orderedServers[i + 1].Port)
                    throw new CacheException(
                        $"Duplicate host:port in the connection string {orderedServers[i].Host}:{orderedServers[i].Port}");


            Servers.Clear();
            foreach (var server in orderedServers) Servers.Add(server);
        }
    }
