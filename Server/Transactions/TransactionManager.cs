...
public class TransactionManager : ITransactionManager
{
                                          ...
                                          SafeDictionary<string, DataStore> dataStores)
    {
        ...
        var lockAcquired =
            _lockManager.TryAcquireWriteLock(transactionId, Constants.DelayForLockInMilliseconds, types.ToArray());
                                 ...
                                 SafeDictionary<string, DataStore> dataStores)
    {
        foreach (var dataRequest in transactionRequest.ChildRequests)
        {
            ...
            switch (dataRequest)
            {
                ...
                default:
                    throw new NotSupportedException(
                        $"Invalid request type in transaction: {dataRequest.GetType().Name}");
            }
