...
public class CommandConnect : CommandBase
{
    internal override IDataClient TryExecute(IDataClient client)
    {
        ...
        try
        {
            ...
            if (Params[0].Trim() == "1") Params[0] = "localhost:48401";

            if (Params[0].Trim() == "2") Params[0] = "localhost:48401+localhost:48402";
            ...
            else // the unique  parameter is a cluster configuration file or a connection string
            {
                ...
                {
                    var channel =
                        new TcpClientChannel(new(4, 1, serverConfig.Host, serverConfig.Port));
