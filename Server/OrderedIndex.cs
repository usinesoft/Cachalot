...
public class OrderedIndex : IndexBase
{
    ...
    public override ISet<PackedObject> GetMany(IList<KeyValue> values, QueryOperator op = QueryOperator.Eq)
    {
        ...
        if (op == QueryOperator.In)
        {
            foreach (var value in values) FindAllEq(value, result);
    ...
    private int CountAllLs(KeyValue key, bool fastEstimate = false)
    {
        ...
        {
            if (!fastEstimate)
                while (index > 0 && _data[index].Values[_keyIndex].CompareTo(key) == 0)
                    index--;
    ...
    private int CountAllGt(KeyValue key, bool fastEstimate = false)
    {
        ...
        //this is useful if the found index is the index of the exact value
        if (!fastEstimate)
            while (index < _data.Count - 1 && _data[index].Values[_keyIndex].CompareTo(key) == 0)
                index++;
