...
public class CommandHelp : CommandBase
{
    private static void DisplayQueryHelp()
    {
        Logger.Write("");
        Logger.Write(
            "Queries are expressed in SQL language. See the 'SQL and LINQ referential' for a complete description");
    ...
    internal override IDataClient TryExecute(IDataClient client)
    {
        ...
        else
        {
            switch (Params[0].ToUpper().Trim())
            {
                ...
                case "SELECT":
                    Logger.Write(
                        "SELECT [DISTINCT] [<property1>, ...] FROM <table> WHERE <query> [ORDER BY <property>] [DESCENDING] [INTO file.json]");
                    DisplayQueryHelp();
                    Logger.Write("");
