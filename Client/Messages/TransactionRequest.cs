...
public class TransactionRequest : Request
{
    ...
    /// <summary>
    ///     This property is assigned later when we find out if more than one node in the cluster participates into a
    ///     transaction
    ...
    public string[] AllCollections
    {
        get
        {
            ...
            foreach (var childRequest in ChildRequests) result.Add(childRequest.CollectionName);
    ...
    /// <returns></returns>
    public SafeDictionary<int, TransactionRequest> SplitByServer([NotNull] Func<KeyValue, int> serverSelector,
                                                                 int serverCount)
    {
        if (serverSelector == null) throw new ArgumentNullException(nameof(serverSelector));

        var result = new SafeDictionary<int, TransactionRequest>(() => new() { TransactionId = TransactionId });
