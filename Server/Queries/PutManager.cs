...
/// </summary>
internal class PutManager : IRequestManager
{
    ...
    public void ProcessRequest(Request request, IClient client)
    {
        if (request is PutRequest putRequest)
        {
            try
            {
                ...
                watch.Start();
                var count = ProcessPutRequest(putRequest);
                watch.Stop();

                _log?.LogActivity(LogEntry.Put, putRequest.CollectionName,
                    (int)(watch.Elapsed.TotalMilliseconds * 1000), $"{putRequest.Items.Count} items");
