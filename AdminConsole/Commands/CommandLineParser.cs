...
public class CommandLineParser
{
    ...
    public CommandBase Parse(string command)
    {
        ...
        {
            result = new() { CmdType = CommandType.Exit, Success = true };
            result.Success = true;
            return result;
        }
        ...
        if (command.Trim().ToLower()
            .StartsWith("longest")) // shorthand for select queries that took longest time to execute server-side
        {
            result = new CommandSelect { CmdType = CommandType.Select };
        ...
        if (Parse(command, "(^SEARCH)\\s+([a-zA-Z0-9\\.]+)\\s+(.+)", atoms))
        {
            ...
            else
            {
                ...
                try
                {
                    ...
                    result.Query = new(typeDescription.CollectionName)
                    {
        ...
        else if (Parse(command, "^CONNECT\\s*(.*)", atoms))
        {
            ...
            if (atoms.Count == 1) result.Params.Add(atoms[0].Trim());
    ...
    private void ParseSelectOrCount(string command, CommandBase result)
    {
        try
        {
            ...
            if (table == null) throw new NotSupportedException("Invalid syntax. No table name or missing FROM clause");


            var schema = GetSchemaByName(table);

            if (schema == null) throw new NotSupportedException($"Unknown table {table}");

            var query = selectNode.ToQuery(schema);

            var into = selectNode.Children.FirstOrDefault(n => n.Token == "into")?.Children.FirstOrDefault();
            if (into != null) result.Params.Add(into.Token);
