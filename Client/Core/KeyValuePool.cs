...
public static class KeyValuePool
{
    ...
    public static KeyValue Pool(KeyValue kv)
    {
        ...
        var poolToUse = kv.Type switch
        {
            ...
            _ => throw new("Should not get here")
        };
        ;

        Interlocked.Increment(ref _requests);
        if (kv.ExtraBytes > 0) Interlocked.Increment(ref _complexValues);
    ...
    {
        for (var i = 0; i < po.Values.Length; i++) po.Values[i] = Pool(po.Values[i]);
    }

    public static void ProcessPackedObjects(IEnumerable<PackedObject> objects)
    {
        foreach (var o in objects) ProcessPackedObject(o);
    }
}