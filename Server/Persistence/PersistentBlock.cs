...
public class PersistentBlock
{
    ...
    {
        if (BeginMarker != BeginMarkerValue) return false;

        if (EndMarker != EndMarkerValue) return false;
    ...
    public bool Read(BinaryReader reader, bool silent = false)
    {
        ...
        try
        {
            ...
            LastTransactionId =
                reader.ReadInt32(); // I know its read twice. But we keep going to preserve compatibility
