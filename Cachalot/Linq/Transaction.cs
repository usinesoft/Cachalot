...
{
    private readonly List<DataRequest> _childRequests = new();
    ...
    public void Put<T>(T item, string collectionName = null)
    {
        ...
        var request = _childRequests
            .Where(r => r.CollectionName == collectionName && r is PutRequest putRequest && !putRequest.HasCondition)
            .Cast<PutRequest>().FirstOrDefault();
