...
public class AdminService : IAdminService
{
    ...
    public string? GetBackupDirectory()
    {
        ...
        if (clusterName != null && _settings.TryGetValue(clusterName, out var dir)) return dir;
    ...
    public string[] GetAvailableBackups()
    {
        ...
        if (dir != null && Directory.Exists(dir))
        {
            ...
            var dirs = Directory.EnumerateDirectories(dir, "????-??-??_??h??_*", SearchOption.TopDirectoryOnly)
                .ToList();
            return dirs.Select(x => x.Trim()).Select(x => new DirectoryInfo(x).Name).OrderByDescending(x => x)
                .ToArray()!;
        }
    ...
    public Guid StartRecreate(string backup)
    {
        ...
        return BackgroundProcess("recreate",
            () => _clusterService.Connector?.AdminInterface().InitializeFromDump(fullPath));
    }
    ...
    private string? ClusterName()
    {
        ...
        if (info.Status == ClusterInformation.ClusterStatus.Ok) return info.ServersStatus[0].ClusterName;
