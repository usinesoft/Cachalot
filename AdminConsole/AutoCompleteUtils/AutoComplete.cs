...
public static class AutoComplete
{
    ...
    private static IList<string> Properties(IList<CollectionSchema> knownTypes, string tableName = null)
    {
        if (tableName != null)
            return knownTypes
                .Where(t => t.CollectionName.Equals(tableName, StringComparison.InvariantCultureIgnoreCase))
                .SelectMany(t => t.ServerSide).Select(k => k.Name).ToList();

        return knownTypes.SelectMany(t => t.ServerSide).Select(k => k.Name).ToList();
    }


    private static IList<string> AfterSelect(IList<CollectionSchema> knownTypes)
    {
        return Properties(knownTypes).Union(new[] { "distinct" }).ToList();
    }


    private static string SmartJoin(params string[] parts)
    {
    ...
    private static string TryGetTableName(string line)
    {
        ...
        {
            if (afterFrom) return token.NormalizedText;

            if (token.NormalizedText == "from") afterFrom = true;
        }

        return null;
    }

    private static string TrimLast(string line)
    {
    ...
    {
        if (!string.IsNullOrWhiteSpace(lineAfter) && !lineAfter.StartsWith(" ")) return new List<string>();
        ...
        if (partsBefore.Length == 2 && !lineBefore.EndsWith(" ")) // command plus the beginning of something
        {
            ...
            if (command == "desc")
                // collection name may be specified as optional parameter for these commands
                return knownTypes.Where(t => t.CollectionName.ToLowerInvariant().StartsWith(toComplete))
                    .Select(t => SmartJoin(command, t.CollectionName)).ToList();

            if (command == "help")
                // collection name may be specified as optional parameter for these commands
                return Commands.Where(c => c != "help" && c.StartsWith(toComplete)).Select(t => SmartJoin(command, t))
                    .ToList();

            if (command == "count" || command == "select")
                return AfterSelect(knownTypes).Where(t => t.ToLowerInvariant().StartsWith(toComplete))
                    .Select(t => SmartJoin(command, t, lineAfter)).ToList();
        }
        ...
        {
            var beforeFrom = lineAfter != null && lineAfter.ToLowerInvariant().Contains("from");

            var afterFrom = lineBefore.ToLowerInvariant().Contains("from");

            var noFrom = !beforeFrom && !afterFrom;
            ...
            if (afterFrom && string.IsNullOrWhiteSpace(lineAfter))
            {
                ...
                if (!lineBefore.EndsWith(" "))
                    return Properties(knownTypes, tableName).Where(p => p.ToLowerInvariant().StartsWith(lastBefore))
                        .Select(p => SmartJoin(TrimLast(lineBefore), p)).ToList();
