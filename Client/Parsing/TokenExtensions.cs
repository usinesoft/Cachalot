...
public static class TokenExtensions
{
    ...
    /// <returns></returns>
    private static IEnumerable<Token> JoinIfKeyword(this IList<Token> original, params string[] separator)
    {
        for (var i = 0; i < original.Count - 1; i++)
        {
            ...
            {
                yield return new() { NormalizedText = joined, Text = joined, TokenType = CharClass.LetterOrDigit };
                i++;
            }
            else
            {
    ...
    public static string Join(this IList<Token> original, int startingAt = 0)
    {
        ...
        for (var i = startingAt; i < original.Count; i++) result.Append(original[i].Text);
