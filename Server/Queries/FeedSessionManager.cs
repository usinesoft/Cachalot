...
{
    private readonly SafeDictionary<Guid, List<PackedObject>> _objectsBySession = new(() => new());
    ...
    public IList<PackedObject> EndSession(Guid sessionId)
    {
        ...
        if (all == null) throw new NotSupportedException("Ending a feed session that was not opened");
