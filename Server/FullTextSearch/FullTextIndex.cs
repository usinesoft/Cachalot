...
public class FullTextIndex
{
    ...
    private Dictionary<LinePointer, LineResult> Find(string query, int maxResults)
    {
        ...
        if (LineProvider != null)
        {
            ...
            Parallel.ForEach(sameLineResult, r =>
            {
                ...
                lock (result)
                {
                    result[r.Key] = new() { Score = value, TokensFound = r.Value.TokensFound };
                }
            });
    ...
    /// </summary>
    private static double ComputeDocumentScore(IList<LineResult> lineResults)
    {
    ...
    private Dictionary<LinePointer, LineResult> SameLineFind(string query)
    {
        ...
        foreach (var pair in foundTokensByLine)
        {
            ...
            result[pair.Key] = new() { Score = scores[pair.Key] = score, TokensFound = pair.Value };
        }
    ...
    {
        private readonly Dictionary<LinePointer, double> _scoreByPointer = new();
