...
public static class SqlExtensions
{
    public static OrQuery ToQuery(this Node node, CollectionSchema schema)
    {
        ...
        if (node.Token is "select" or "count") result = SelectToQuery(node, schema);

        QueryHelper.OptimizeQuery(result);

        if (node.Token == "count") result!.CountOnly = true;
    ...
    private static OrQuery SelectToQuery(this Node node, CollectionSchema schema)
    {
            ...
            foreach (var column in projection.Children)
            {
                ...
                if (name != "*")
                {
                    ...
                    else
                    {
                        ...
                        query.SelectClause.Add(new() { Name = name, Alias = alias });
                    }
                }
            }
        ...
        if (where != null) ParseWhere(where, query, schema);
        ...
        if (order != null)
        {
            ...
            if (order.Children.Count == 2) query.OrderByIsDescending = true;
        }
        ...
        if (take != null)
        {
            var count = int.Parse(take.Children.FirstOrDefault()?.Token ?? string.Empty);
    ...
    private static void ParseWhere(Node where, OrQuery query, CollectionSchema schema)
    {
        ...
        if (orNodes.Count != 1) throw new NotSupportedException("Query too complex");
            ...
            foreach (var node in andNode.Children)
            {
                ...
                if (op is QueryOperator.In or QueryOperator.NotIn)
                {
                    ...
                    if (metadata != null)
                    {
                        ...
                        {
                            var value = JExtensions.SmartParse(val);
                            values.Add(new(value));
                        }


                        andQuery.Elements.Add(new(metadata, values, op));
                    }
                    else
                    {
                        throw new NotSupportedException(
                            $"Can not parse query after IN operator. {operands[0]} is a server-side value");
                    }
                }

                else if
                    (op is QueryOperator.Contains
                     or QueryOperator
                         .NotContains) // for contains operator the collection property should be at the left side
                {
                    var metadata = schema.KeyByName(operands[0]);

                    var value = JExtensions.SmartParse(operands[1]);

                    andQuery.Elements.Add(new(metadata, new KeyValue(value), op));
                }
                else if (op is QueryOperator.StrStartsWith or QueryOperator.StrEndsWith
                         or QueryOperator.StrContains) // for string operators the property should be at the left side
                {
                    var metadata = schema.KeyByName(operands[0]);
                ...
                else if (operands.Count == 2) // binary operators
                {
                    ...
                    else // try value first
                    {
                        ...
                        if (metadata == null)
                            throw new NotSupportedException(
                                $"Can not parse query. Neither {operands[0]} nor {operands[1]} is a server-side value");


                        var value = JExtensions.SmartParse(operands[0]);

                        andQuery.Elements.Add(new(metadata, new KeyValue(value), Reverse(op)));
                    }
                }
            }
