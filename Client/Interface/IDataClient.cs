...
internal interface IDataClient : IDisposable
{
    ...
    void FeedMany(string collectionName, IEnumerable<PackedObject> items, bool excludeFromEviction,
    ...
    ///     If schema is different that the one registered on the server this will trigger reindexing which may take
    ///     significant time
    /// </summary>
    /// <param name="collectionName"></param>
    ...
    /// <summary>
    ///     Acquire a server side lock and return a session id if successful (default(guid) otherwise). The session id can be
    ///     used for multiple calls to <see cref="GetMany" />
