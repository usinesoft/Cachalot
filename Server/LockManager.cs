...
public class LockManager : ILockManager
{
    ...
    /// </summary>
    private readonly SafeDictionary<Guid, Session> _activeSessions = new(null);
    ...
    /// </summary>
    private readonly SafeDictionary<ServerSideLock, DateTime> _locksCurrentlyTaken = new(null);
    ...
    public bool TryAcquireReadLock(Guid sessionId, int delayInMilliseconds, params string[] resourceNames)
    {
        ...
        if (result && sessionId != default)
        {
            ...
            if (session == null)
                _activeSessions.Add(sessionId, new(resourceNames));
            else
    ...
    public bool TryAcquireWriteLock(Guid sessionId, int delayInMilliseconds, params string[] resourceNames)
    {
        ...
        if (result && sessionId != default)
        {
            var session = _activeSessions.TryGetValue(sessionId);
            if (session == null)
                _activeSessions.Add(sessionId, new(resourceNames) { IsWriteLock = true });
            else
                throw new NotSupportedException("The lock session has already been open");
        }
    ...
    private class Session
    {
        ...
        {
            LockedResources = new(resourceNames);
        }
