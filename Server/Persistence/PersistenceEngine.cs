...
public class PersistenceEngine : ITransactionLog
{
    ...
    public void Start()
    {
        _storage = Container != null
            ? new(new ObjectProcessor(Container), WorkingDirectory)
        ...
        TransactionLog = new(WorkingDirectory);
    ...
    {
        if (resetStorage)
            _storage = new(new NullObjectProcessor(), WorkingDirectory);
        else
    ...
    ///     it is not tokenized yet. While storing an item from the transaction log in the persistent storage, get the
    ///     tokenized full-text
    ///     if available or tokenize it otherwise. It is important to avoid tokenization while reloading the database
    /// </summary>
    /// <param name="item"></param>
    /// <returns></returns>
    private PackedObject GetItemWithTokenizedFullText(PackedObject item)
    {
